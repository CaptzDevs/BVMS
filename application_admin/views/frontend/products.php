  ...
  <main>
    <section class="section-banner">
      <div class="banner-cover">
        <h1 class="banner-header"> <?= isset($productList[0]['series_name']) ? $productList[0]['model_name'].' : '.$productList[0]['series_name'] : $label   ?></h1>
      ...
      <div class="banner-media">
        <?php if (isset($productList[0]['series_image']) && strlen($productList[0]['series_image']) > 0) { ?>
          <img src="<?= base_url("/uploads/image/") . $productList[0]['series_image'] ?>" alt="" />
        <?php } else { ?>
    ...
    <div class="container-section">
      <section class="section-space mt-3">
        ...
        <div class="space-item">
          ...
          <div class="space-text">
            Buddymotor - Auto Parts, Lighting, and Accessories. dsadadas
          </div>
        </div>
        <div class="line"></div>
      </section>
      ...
      <h3 style="width: 100%; padding-left:30px;border-left:5px solid white; padding-left: 10px;">
            <?php if(strlen( $productList[0]['series_image'] ) > 0){ ?>
              <img class="series_logo" src="<?= base_url("/uploads/image/") . $productList[0]['series_image'] ?>" alt="">
              <?php }else{ ?>
                <?= $productList[0]['series_name'] ?>
            <?php } ?>
          </h3>
      ...
      $dataTotal = ceil($dataTotal / (int) $limit);
          ...
          <div class="f-center gap-2 p-3">
            <nav aria-label="Page navigation example">
              <ul class="pagination">
                ...
                <?php if (isset($page) &&  (int) $page > 1) { ?>

                  <li class="page-item">
                    <a class="page-link" href="<?= base_url($LANG . $path . "?p=" . (((int) $page) - 1)) ?>" aria-label="Previous">
                ...
                <?php for ($i = 1; $i <= $dataTotal; $i++) {  ?>
                  <?php $disabled =  isset($page) && $page != $i ? 1 : 0 ?>


                  <li class="page-item">
                    <a class="page-link" <?php if (!$disabled) { ?> style="opacity:50%;cursor:no-drop" ; <?php } ?> <?php if ($disabled) { ?> href="<?= base_url($LANG . $path . "?p=" . $i);
                                                                                                                                                  } ?>">
                      <?= $i ?></a>
                  </li>
                <?php } ?>

                <?php if (isset($page) &&  (int) $page < $dataTotal) { ?>

                  <li class="page-item">
                    <a class="page-link" href="<?= base_url($LANG . $path . "?p=" . (((int) $page) + 1)) ?>" aria-label="Next">
      ...
      <!--   <section class="section-space">
        <div class="f-center gap-2 p-3">
          <a href="<?= base_url() . $LANG ?>/products" class="btn mt-1 btn-discovery product-more-btn">
