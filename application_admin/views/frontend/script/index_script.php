 ...
 */
        async function loadData(parameters) {
                ...
                $.ajax({
                    url: "<?= base_url('admin.php/Control/getCCTVTotal?date=') ?>",
                    dataType: "json",
                    method: "GET",
                    cache: false,
        ...
        function rederChart(id, label, dataSet ,title) {
        ...
        async function loadHistoryData(date) {
            $.ajax({
                url: "<?= base_url('admin.php/Control/getCCTVTotalHistory') ?>",
                dataType: "json",
                method: "GET",
                cache: false,
                data : {
                ...
                success: async function(results) {
                    ...
                    if (results.length > 0) {
                        ...
                        let label = results.map(item => item.date)
                        let dataHttp = results.map(item => item.issue_http)
                        let dataHttps = results.map(item => item.issue_https)
                        let dataTotal = results.map(item => item.avaliavle_total)
                        ...
                        rederChart('totalChart', label, dataSetTotal , "Total CCTV");
                        rederChart('issueHTTPChart', label, dataSetHttp, "CCTV Issue (HTTP)");
                        rederChart('issueHTTPSChart', label, dataSetHttps,"CCTV Issue (HTTPS)");
    ...
    $(document).ready(function($) {
        ...
        async function loadData(parameters) {
            ...
            ${sess.fname + " "+ sess.lname } (Logout)
                ...
                $.ajax({
                    url: "<?= base_url('admin.php/Control/getMapData') ?>",
                    dataType: "json",
                    method: "GET",
                    cache: false,
                    success: async function(results) {
                        ...
                        console.log(results)
                        let branchSet =sess.branch.split(',')
        ...
        function loadฺSidebarData(id) {
            return $.ajax({
                url: "<?= base_url('admin.php/Control/getMapData/') ?>" + id,
                dataType: "json",
                method: "GET",
                cache: false,
                error: function(e) {
        ...
        async function openInfobox(parameters) {
