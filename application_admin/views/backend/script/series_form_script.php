...
<script>
    ...
    function deleteImage(imageId) {
        return $.ajax({
            url: '<?= base_url("/Control/deleteImageSeries") ?>',
            type: 'POST',
            data: {
                id: imageId
            },
    ...
    $("#btn_save_appointment").click(async e => {
        ...
        if (form_appointment.isValid()) {
            $("#btn_save_appointment").text('checking...')
            $("#btn_save_appointment").prop('disabled', true)

            await saveData()

            $("#btn_save_appointment").text('Save Series')
    ...
    function editAppointment() {
        ...
        return $.ajax({
            url: '<?= base_url("/Control/editSeriesDetail") ?>',
            type: 'POST',
    ...
    $("#btn_edit_appointment").click(async e => {
        ...
        if (form_appointment.isValid()) {
            $("#btn_edit_appointment").text('Checking...')
            $("#btn_edit_appointment").prop('disabled', true)

            await editAppointment()

            $("#btn_edit_appointment").text('Save Series')
...
<script>
    ...
    let dropzoneImage = initDropZone("dz-image", "<?php echo base_url('/Control/uploadSeriesImage') ?>", 'image/*');
