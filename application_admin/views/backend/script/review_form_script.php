...
<script>
        ...
        return $.ajax({
            ...
            success: function(response, status) {
                ...
                if (response == 1) {
                    ...
                    location.href = "<?= base_url('/Admin/Reviews') ?>";
                }
            ...
            success: function(response, status) {
               ...
               /*  let fileLength = dropzoneFile.getQueuedFiles().length */
                ...
                if (imageLength == 0/*  && fileLength == 0 */) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'บันทึกรีวิวแล้ว',
                        confirmButtonText: 'ตกลง',
                    }).then((result) => {
                        if (result.isConfirmed) {
                            location.href = "<?= base_url("/Admin/Review/") ?>" + response
                        }
                        setTimeout(() => {
                            location.href = "<?= base_url("/Admin/Review/") ?>" + response
                        }, 1000);
    ...
    function editAppointment() {
        ...
        return $.ajax({
            url: '<?= base_url("/Control/editReview") ?>',
            type: 'POST',
    ...
    $("#btn_edit_appointment").click(async e => {
        ...
        if (form_appointment.isValid()) {
            ...
            $("#btn_edit_appointment").text('บันทึกรีวิว')
    ...
    function initDropZone(elemId, uploadUrl, accept) {
        var myDropzone = new Dropzone("#" + elemId, {
            ...
            success: function(file, response) {
                ...
                setTimeout(() => {
                    location.href = "<?= base_url("/Admin/Review/") ?>" + response.file.ref_id
                }, 1000);
