			...
			<tbody>
				<?php foreach ($result as $key => $value) { 
					...
					<!-- 	<td>  <input type="checkbox" class="hostbox" data-branchid= "<?= $branchID ?>" data-hostid="<?= $value['hostid'] ?>" /> </td> -->
						<td>
						<input type="checkbox" class="hostbox"
						 	data-branchid="<?= $branchID ?>"
						  	data-hostid="<?= $value['hostid'] ?>"
							data-type="<?php echo in_array($value['hostid'], array_column($result2, 'hostid')) ? 'init' : 'save'; ?>"
						   <?php echo in_array($value['hostid'], array_column($result2, 'hostid'))   ? 'checked' : ''; ?>
